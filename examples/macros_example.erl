...
-define(oneof(Needle_, Elements_),
    ?procedural(oneof(Needle, Elements), begin
...
simple_procedural(MaybeFamous) when
    is_integer(MaybeFamous) orelse is_float(MaybeFamous)
->
...
-define(expect(Expected, Guard),
    ?hygienic(expect(Expected, Guard), begin
        case (Expected) of
            Guard ->
                ok;
            Value ->
